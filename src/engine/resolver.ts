...
export function resolveDashboard(
  ...
  if (!declaration.allowedSubjects.includes(context.subjectType)) {
    throw new SubjectAccessError(
      `Subject type '${context.subjectType}' is not allowed for dashboard '${declaration.dashboardId}'`
    );
  }
  ...
  if (
    ...
    throw new TenantIsolationError(
      `Tenant '${context.tenantId}' is not allowed for dashboard '${declaration.dashboardId}'`
    );
  }

  if (
    declaration.allowedPartners &&
    declaration.allowedPartners.length > 0 &&
    context.partnerId &&
    !declaration.allowedPartners.includes(context.partnerId)
  ) {
    throw new PartnerIsolationError(
      `Partner '${context.partnerId}' is not allowed for dashboard '${declaration.dashboardId}'`
    );
  }

  if (entitlementSnapshot.tenantId !== context.tenantId) {
    throw new TenantIsolationError(
      `Entitlement snapshot tenant '${entitlementSnapshot.tenantId}' does not match context tenant '${context.tenantId}'`
    );
  }
